...
DFT <- data.frame(X3A_CODE = toupper(SRG.m[,1]), Region = toupper(SRG.m[,2]), geo = SRG.m[,4],
                  Y2017 = DF$X2017, Y2016 = DF$X2016, Y2015 = DF$X2015, Y2014 = DF$X2014, Y2013 = DF$X2013)
...
sum_year <- summarise(group_by(DFM, Country), sum_2017 = sum(Y2017, na.rm = T), sum_2016 = sum(Y2016, na.rm = T), sum_2015 = sum(Y2015, na.rm = T)
