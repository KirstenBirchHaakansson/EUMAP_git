...
input_path <- "Q:/scientific-projects/eu-data-collection/Work_Plan/2019/scripts/EUMAP_table_1a/joels_program"
ASFIS   <- read.table(file.path(sharePath, 'ASFIS_sp_Feb_2016.txt'), header=TRUE, sep="\t", as.is=TRUE)

setwd('M:/DCF/AP/2017/Programs/Tabel_1A')

DF <- read.table(file.path(sharePath,"fish_ca_atl27.tsv"),header=TRUE, sep='\t', as.is=TRUE)  # Atlantique NE
GEO <- read.table(file.path(sharePath,'geo.def'),header=TRUE,sep=";", as.is=TRUE)
...
DFT <- data.frame(X3A_CODE = toupper(SRG.m[,1]), Region = toupper(SRG.m[,2]), geo = SRG.m[,4],
                  Y2015 = DF$X2015, Y2014 = DF$X2014, Y2013 = DF$X2013)
DFM <- merge(DFT, GEO, all.x=TRUE)
DFM$Y2013 <- as.numeric(as.character(DFM$Y2013))
DFM$Y2014 <- as.numeric(as.character(DFM$Y2014))
DFM$Y2015 <- as.numeric(as.character(DFM$Y2015))
...
subset(DFM, X3A_CODE %in% c("ARE","ARO","ARU","ARY","ARG") & Region!="27" & geo=="DK")
